...
export default class CameraScreenBase extends Component {
    ...
    isCaptureRetakeMode() {
        return !!(
            this.props.allowCaptureRetake &&
            !_.isUndefined(this.state.imageCaptured)
        );
    }
    ...
    getCameraOptions() {
        ...
        if (this.props.cameraRatioOverlay) {
            ...
            cameraOptions.ratioOverlayColor =
                overlay.color || OVERLAY_DEFAULT_COLOR;
            ...
            >
                ...
                >
                    <Text style={styles.ratioBestText}>
                        Your images look best at a {this.state.ratios[0] || ''}{' '}
                        ratio
                    </Text>
                    <TouchableOpacity
                        style={{
    ...
    async onButtonPressed(type) {
        ...
        if (captureRetakeMode) {
            if (type === 'left') {
                ...
                const result = await GalleryManager.saveImageURLToCameraRoll(
                    this.state.imageCaptured.uri
                );
                ...
                this.setState(
                    {
                        imageCaptured: undefined,
                        captureImages: _.concat(
                            this.state.captureImages,
                            savedImage
                        ),
                    },
                    () => {
                        this.sendBottomButtonPressedAction(
                            type,
                            captureRetakeMode
                        );
                    }
    ...
    renderBottomButton(type) {
        ...
        if (type === 'right') {
            showButton =
                this.state.captureImages.length || this.isCaptureRetakeMode();
        }
        if (showButton) {
            const buttonNameSuffix = this.isCaptureRetakeMode()
                ? 'CaptureRetakeButtonText'
                : 'ButtonText';
            const buttonText = _(this.props).get(
                `actions.${type}${buttonNameSuffix}`
            );
            return (
                <TouchableOpacity
                    style={[
    ...
    renderBottomButtonsFrame() {
        return (
            !this.props.hideControls && (
                <SafeAreaView
                    style={styles.bottomButtonsFrame}
                >
                    {this.renderBottomButton('left')}
                    {this.renderCaptureButton()}
                    {this.renderBottomButton('right')}
    ...
    async onSetFlash() {
        this.currentFlashArrayPosition =
            (this.currentFlashArrayPosition + 1) % 3;
